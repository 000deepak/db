...
| `$min`       | Gets the minimum value of the corresponding field across all documents.     | `db.myCollection1.aggregate([{$group: {_id: "$by_user", num_tutorial: {$min: "$likes"}}}])` |
| `$max`       | Gets the maximum value of the corresponding field across all documents.     | `db.myCollection1.aggregate([{$group: {_id: "$by_user", num_tutorial: {$max: "$likes"}}}])` |
| `$push`      | Inserts the value into an array in the resulting document.                  | `db.myCollection1.aggregate([{$group: {_id: "$by_user", url: {$push: "$url"}}}])`            |
| `$addToSet`  | Inserts the value into an array in the resulting document but prevents duplicates. | `db.myCollection1.aggregate([{$group: {_id: "$by_user", url: {$addToSet: "$url"}}}])`       |
| `$first`     | Gets the first document according to the grouping criteria.                 | `db.myCollection1.aggregate([{$group: {_id: "$by_user", first_url: {$first: "$url"}}}])`    |
| `$last`      | Gets the last document according to the grouping criteria.                  | `db.myCollection1.aggregate([{$group: {_id: "$by_user", last_url: {$last: "$url"}}}])`      |
